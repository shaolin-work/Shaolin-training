...
const STORAGE_KEY = 'shaolin-path-v64';
...
function getImageCandidates(src){
  ...
  return [...new Set([clean, `assets/${rootName}`, rootName, `./assets/${rootName}`, `./${rootName}`])];
}
function getVisibleLibraryModules(){
  const q = (state.librarySearch || '').trim().toLowerCase();
  if (!q) return MODULES;
  return MODULES.filter(mod => [mod.title, mod.area, mod.mission, mod.cues, mod.steps].join(' ').toLowerCase().includes(q));
}
function getCurrentLibraryModule(){
  const visible = getVisibleLibraryModules();
  return visible.find(m => m.id === state.selectedModuleId) || MODULES.find(m => m.id === state.selectedModuleId) || MODULES[0];
}
function getCurrentStreak(){
  let streak = 0;
  for (let day = state.currentProgramDay - 1; day >= 1; day--){
    const item = state.dayCompletion[`day-${day}`];
    if (item && item.completed) streak++;
    else break;
  }
  return streak;
}
function openImageModal(index){
  const mod = getCurrentLibraryModule();
  const images = mod.imageFiles || [];
  if (!images.length) return;
  state.modalImageIndex = Math.max(0, Math.min(index, images.length - 1));
  const src = images[state.modalImageIndex];
  const candidates = getImageCandidates(src);
  const img = document.getElementById('imageModalImg');
  img.src = candidates[0] || '';
  img.dataset.fallbacks = JSON.stringify(candidates.slice(1));
  img.alt = mod.images?.[state.modalImageIndex] || mod.title;
  img.onerror = () => {
    let fallbacks = [];
    try { fallbacks = JSON.parse(img.dataset.fallbacks || '[]'); } catch(e) {}
  ...
  document.getElementById('imageModalTitle').textContent = mod.title;
  document.getElementById('imageModalCaption').textContent = mod.images?.[state.modalImageIndex] || '';
...
function shiftImageModal(step){
  const mod = getCurrentLibraryModule();
  const images = mod.imageFiles || [];
  if (!images.length) return;
  const next = (state.modalImageIndex + step + images.length) % images.length;
  openImageModal(next);
}
...
function renderLibrary(){
  const visibleModules = getVisibleLibraryModules();
  const mod = getCurrentLibraryModule();
  document.getElementById('librarySearch').value = state.librarySearch || '';
  document.getElementById('moduleList').innerHTML = visibleModules.map(modItem => {
    const unlocked = getCurrentLevel() >= modItem.level;
    return `<button class="module-item ${state.selectedModuleId === modItem.id ? 'active' : ''}" data-module-id="${modItem.id}"><strong>${modItem.title}</strong><div class="muted">${modItem.area} • Level ${modItem.level} • ${unlocked ? 'Unlocked' : 'Locked'}</div></button>`;
  }).join('') || '<div class="card">No library items matched that search.</div>';
  ...
  const imageFiles = mod.imageFiles || [];
  document.getElementById('moduleImages').innerHTML = imageFiles.length
    ? imageFiles.map((src, idx) => {
        const candidates = getImageCandidates(src);
        const first = candidates[0] || '';
        const backupAttr = escapeHtml(JSON.stringify(candidates.slice(1)));
        const label = escapeHtml(mod.images?.[idx] || mod.title);
        return `<button type="button" class="card module-image-card open-image-btn" data-image-index="${idx}">
          <img src="${first}" data-fallbacks='${backupAttr}' alt="${label}" style="width:100%;height:auto;border-radius:12px;display:block;">
          <div class="muted" style="margin-top:8px;">${label}</div>
        </button>`;
      }).join('')
    : mod.images.map(label => `<div class="card">${escapeHtml(label)} image placeholder</div>`).join('');

  document.querySelectorAll('#moduleImages img').forEach(img => {
    img.onerror = () => {
      let fallbacks = [];
      try { fallbacks = JSON.parse(img.dataset.fallbacks || '[]'); } catch(e) {}
      const next = fallbacks.shift();
      if (next) {
        ...
        img.parentElement.outerHTML = `<div class="module-image-missing"><strong>Image not found</strong><div class="small-muted">Upload this SVG into the assets folder or repo root.</div></div>`;
      }
    };
  });
}
...
document.addEventListener('click', (e) => {
  const btn = e.target.closest('.open-image-btn');
  if (btn){
    openImageModal(Number(btn.dataset.imageIndex || 0));
    return;
  }
  if (e.target.id === 'imageModalClose' || e.target.dataset.closeModal === 'true'){
    closeImageModal();
    return;
  }
  if (e.target.id === 'imageModalPrev'){
    shiftImageModal(-1);
    return;
  }
  if (e.target.id === 'imageModalNext'){
    shiftImageModal(1);
    return;
  }
});
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'ArrowLeft') shiftImageModal(-1);
  if (e.key === 'ArrowRight') shiftImageModal(1);
});
